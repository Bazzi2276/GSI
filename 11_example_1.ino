...
#define _DUTY_MIN 450 // servo full clockwise position (0 degree)
...
void loop() {
  ...
  if (dist_raw < _DIST_MIN) {
    ...
    dist_raw = dist_prev;       
    dist_ema = _EMA_ALPHA * dist_raw + (1 - _EMA_ALPHA) * dist_ema;
    myservo.writeMicroseconds(_DUTY_MAX);    // Cut higher than maximum
    digitalWrite(PIN_LED, 1);       // LED OFF
  } else {    // In desired Range
    digitalWrite(PIN_LED, 0);       // LED ON      
    dist_prev = dist_raw;
    dist_ema = _EMA_ALPHA * dist_raw + (1 - _EMA_ALPHA) * dist_ema;
    myservo.writeMicroseconds(95.0 / 9.0 * dist_ema - 1350);
    Serial.print(95.0 / 9.0 * dist_ema - 1350);
  }
